...
int My_I2C_Write(int length, int reg, unsigned char *data, int address) {
    int returnValue = -1;
...
int I2C_Write(int NumberOfBytes, int RegAddress , unsigned char *TxBuffer)
{
	...
	res = My_I2C_Write(NumberOfBytes, RegAddress, TxBuffer, STC3117_SLAVE_ADDRESS);
...
int My_I2C_Read(int length, int reg, unsigned char *output, int address) {
    int returnValue = 0;
    int counter = 0;
    int temp = 0;
...
{
	int res=-1;
	res = My_I2C_Read(NumberOfBytes, RegAddress, RxBuffer, STC3117_SLAVE_ADDRESS);
...
int GasGauge_Initialization(STC3117_ConfigData_TypeDef *ConfigData, STC3117_BatteryData_TypeDef *BatteryData)
{
  ...
  /*BatteSTC3117_ReadRamDatary presence status init*/
...
/*   This section is the start of the code that will actually run                   */
/* -------------------------------------------------------------------------------- */
